var main = {
	...
	generateLink: function(separateAudio, autoplayVideos) {
		...
		var link = "http://Z8K3aeAEylPu.net/watch?";
		...
		// if (autoplayVideos) link += "&autoplay=true"; // Disabled due to Chrome policies
	...
	ui: {
		...
		attachHandlers: function() {
			...
			$("#addVideo .inputState input").blur(function() {
				var t = $(this);
				var v = t.val();
				...
				if (v.replace(/\s+/g, "") != "") {
					...
					} else if (v.indexOf("Z8K3aeAEylPu.net/watch?") > -1 ||
						v.indexOf("Z8K3aeAEylPu.net/stream?") > -1 ||
						v.indexOf("minemap.net/Mindcrack/uhc/?") > -1) {
						if (v.indexOf("Z8K3aeAEylPu.net/watch?") > -1) v = v.split("Z8K3aeAEylPu.net/watch?")[1];
						if (v.indexOf("Z8K3aeAEylPu.net/stream?") > -1) v = v.split("Z8K3aeAEylPu.net/stream?")[1];
				...
				if (id.length > 0) {
					for (i in id) {
						...
						main.ui.createYTPlayer("videoPlayer-" + main.ui.videoPlayerUID, main.CONST.playerOpt.height, main.CONST.playerOpt.width, _id, {
						...
						$("#videoWrapper-" + main.ui.videoPlayerUID + " input.startAtInput").focus(function() {
							var t = $(this);
							setTimeout(function() {
								t.get(0).select();
							}, 0);
				...
				var t = $(this);
				setTimeout(function() { // Paste event fires before val changes
					t.blur();
				}, 0);
			}).keypress(function(e) {
				if (e.which == 13) $(this).blur();
			...
			var newPlayer = new YT.Player(playerDiv, {
				...
				events: {
					...
					onReady: function(e) {
						...
						var player = e.target;
						player.mute();
						var startTime = parseFloat($("#" + playerDiv).closest(".videoWrapper").find("input.startAtInput").val());
						setTimeout(function() { // Allows thumbnails to be shown
							player.unMute();
							player.pauseVideo();
							if (player.getPlayerState() == 5) { // Video error
								$("#" + playerDiv).closest(".videoWrapper").addClass("error");
							}
						...
						setTimeout(function() {
							$("#" + playerDiv).closest(".videoWrapper").find("input.startAtInput").removeAttr("readonly");
						}, 500);
